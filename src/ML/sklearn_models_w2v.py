...
def Train_and_Test(Train_features, Test_features, Train_label, Test_label):
    ...
    for model in models:
        ...
        logger.info(Embedding_flag+"_"+model_name+'_' +
            'test precision_score %s' % metrics.precision_score(Test_label,
                    Test_predict_label, average='micro'))
        # 输出模型预测错误的类别信息
        # 找出该模型分类错误的样本下标
        ...
        for k in range(len(predict_error_list)):
            ...
            logger.info("预测错误样本的text{},预测标签:{},样本的真实标签:{}".format(
                ...
                labelIndexToName[int(
                    Test_predict_label[predict_error_list[k]])],
