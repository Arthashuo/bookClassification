...
def Train_and_Test(Train_features, Test_features, Train_label, Test_label):
    ...
    models = [
        ...
        lgb.LGBMClassifier(objective='multiclass', n_jobs=10,
                        num_class=33, num_leaves=30, reg_alpha=10, reg_lambda=200,
    ...
    for model in models:
        ...
        print(Embedding_flag+"_"+model_name+'_'+'test precision_score %s' %
            metrics.precision_score(Test_label, Test_predict_label, average='micro'))

        logger.info(Embedding_flag+"_"+model_name+'_'+'test precision_score %s' %
            metrics.precision_score(Test_label, Test_predict_label, average='micro'))
        # 输出模型预测错误的类别信息
        # 找出该模型分类错误的样本下标
        ...
        for k in range(len(predict_error_list)):
            if int(Test_predict_label[predict_error_list[k]]) not in Dict02.keys():
                Dict02[int(Test_predict_label[predict_error_list[k]])] = list(
